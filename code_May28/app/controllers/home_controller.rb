...
class HomeController < ApplicationController
   ...
   before_action :authenticate_user!, :except => [:index, :test,:addSession,:BurgerProfile,:CheckEmail,:reviewuser,:addlocation,:actionSendMailMutile,:getAutocomplete]
  ...
  def user_meta
   ...
   def getallCountry
     data=Country.all().order(name: :asc)
     render :json => { :data => data }
   end
  def update_profile
...
def uploadImgaes
  ...
  def login
      ...
      if !@getLocation.blank? && !request.post?
          ...
          @data=Product.joins(:restaurant).joins("LEFT OUTER JOIN reviews rw on rw.product_id=products.id").select("products.id,products.images_file_name,products.name as productname,restaurants.name as restaurantsname,AVG(rw.totalpoint) as avgg").where(restaurants: {city_id: @city_id}).group('products.id,products.images_file_name,products.name,restaurants.name').order('avgg DESC')
        ...
        if @getLocation.blank? && request.post?
          ...
          if @city.present? && !params[:idcomponennt].present?
            ...
            if @city.present? && params[:idcomponennt].present?
              ...
              @data=Product.joins(product_components: [{component_value: :component }]).joins(:restaurant).joins("LEFT OUTER JOIN reviews rw on rw.product_id=products.id").select("products.id,products.images_file_name,products.name as productname,restaurants.name as restaurantsname,AVG(rw.totalpoint) as avgg").group('products.id,products.images_file_name,products.name,restaurants.name').order('avgg DESC').distinct
          ...
          if !@getLocation.blank? && request.post?
            ...
            # khong lay va khong post
              ...
              #@location=City.where(country_id:countryid).order(name: :asc)
              #@city_id=@location.first().id
              @data=Product.joins(:product_components).joins(:restaurant).joins("LEFT OUTER JOIN reviews rw on rw.product_id=products.id").select("products.id,products.images_file_name,products.name as productname,restaurants.name as restaurantsname,AVG(rw.totalpoint) as avgg").distinct.where(restaurants: {city_id: @city_id}).group('products.id,products.images_file_name,products.name,restaurants.name').order('avgg DESC')
